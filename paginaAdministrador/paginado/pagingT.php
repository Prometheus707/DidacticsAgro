...
/* querys */
$count="SELECT COUNT(*) FROM tda where idTipoTdas='4'";
 ...
 mysqli_set_charset($conecta,"utf8");
  ...
  $rs = mysqli_query($conecta, "SELECT DISTINCT  MAX(idTda) FROM tda where idTipoTdas='4'");
  if ($row = mysqli_fetch_row($rs)) {
  $id = trim($row[0]);


  }
  $select = "SELECT * FROM tda inner join tipotda on tipotda.idTipoTda=tda.idTipoTdas where idTipoTdas='4' ORDER BY idTda DESC";
  ...
  while($rows = $c->fetch_assoc())
  {
    ...
    echo "<td> <a href='actualizarTransfe.php?id=".$rows['idTda']."'> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Actualizar'></a></td>";
    
    echo "<td id='t9'> <a href='eliminarTransfe.php?id=".$rows['idTda']."' onclick=\"return confirmar('¿Está seguro que desea eliminar el registro?')\"> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Eliminar'></a></td>";
