...
/* querys */
$count="SELECT COUNT(*) FROM reflexioncierrepdf";
 ...
 mysqli_set_charset($conecta,"utf8");
  ...
  $rs = mysqli_query($conecta, "SELECT DISTINCT  MAX(idReflexionCPdf) FROM reflexioncierrePdf");
  if ($row = mysqli_fetch_row($rs)) {
  $id = trim($row[0]);


  }
  $select = "SELECT * FROM reflexioncierrepdf  ORDER BY idReflexionCPdf DESC";
  $select .= " LIMIT $init, $limit_end";
  $c = $conecta->query($select);
  while($rows = $c->fetch_assoc())
  {
    if($id == $rows['idReflexionCPdf']) {
    echo "<tr>";
    echo "<td>".$rows['TituloRefPdf']."</td>";
    echo "<td>".$rows['descripcionRefPdf']."</td>


          <td style='text-align:center'><a href=../pdf/".$rows["pdfRefC"]."><img src='../images/ver.png'></a></td>
          <td> <a href='actualizarCierrePdf.php?id=".$rows['idReflexionCPdf']."'> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Actualizar'></a></td>";


    echo "</tr>";
    }
    if($rows['idReflexionCPdf'] < $id ) {
      echo "<tr>";
      echo "<td>".$rows['TituloRefPdf']."</td>";
      echo "<td>".$rows['descripcionRefPdf']."</td>
