...
$count="SELECT COUNT(*) FROM iniciosesion";
 ...
 mysqli_set_charset($conecta,"utf8");
  ...
  {
    if($id == $rows['idIniciosesion']) {
    echo "<tr>";
    echo "<td>".$rows['nombreInicio']."</td>";
    echo "<td>".$rows['descripcionInicioS']."</td>


          <td style='text-align:center'><a href=../pdf/".$rows["pdfInicioS"]."><img src='../images/ver.png'></a></td>
          <td> <a href='actualizarSesion.php?id=".$rows['idIniciosesion']."'> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Actualizar'></a></td>
          
          <td> <a href='eliminarSesion.php?id=".$rows['idIniciosesion']."'> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Eliminar'></a></td>
