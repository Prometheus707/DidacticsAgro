...
/* querys */
$count="SELECT COUNT(*) FROM motivdores";
 ...
 mysqli_set_charset($conecta,"utf8");
  ...
  echo "<tbody>";
  $rs = mysqli_query($conecta, "SELECT DISTINCT  MAX(idMotivadores) FROM motivdores");
  ...
  $select = "SELECT * FROM motivdores ORDER BY idMotivadores DESC";
  $select .= " LIMIT $init, $limit_end";
  $c = $conecta->query($select);
  while($rows = $c->fetch_assoc())
  {
    if($id == $rows['idMotivadores']) {
    echo "<tr>";
    echo "<td>".$rows['tituloMoti']."</td>";
    echo "<td style='white-space:pre-line'>".$rows['descripcionMoti']."</td>
          <td > <img    class='zoom' src='../images/".$rows["imagenMoti"]." '> </td>
          <td> <a href='actualizarJolts.php?id=".$rows['idMotivadores']."'> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Actualizar'></a></td>
           <td> <a href='eliminarJolts.php?id=".$rows['idMotivadores']."'> <input  class='btn   btn-sm' style='color: green; font-size: 18px' type='button' value=' Eliminar'></a></td>";


    echo "</tr>";
    }
    if($rows['idMotivadores'] < $id) {
      echo "<tr>";
      echo "<td>".$rows['tituloMoti']."</td>";
      echo "<td style='white-space:pre-line'>".$rows['descripcionMoti']."</td>
            <td > <img    class='zoom' src='../images/".$rows["imagenMoti"]." '> </td>
